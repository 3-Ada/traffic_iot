...
So first stuff to do is to check if we have any missing values.
...
method was each duplicated timestamp change to average of previous and
next value, repeat it until there are no more duplicates. However, due
to this method the complexity and dimensionality of data would increase
and also it would be chalenging to make interval between data points
equal in this case.
...
The periodic behavior can be seen in the previous graph. A better
...
*One picture is worth of a thousand words*
...
The dataset has too much data to be displayed in understandable way. Due
to this fact, the library plotly has been used. That package helps to
create interactive plots, that can be zoom in.
...
can be assumed. There is no trend. Also the dataset has period of
unrecorded data.
...
the data has daily nature. Let's create a column with hour.
...
By using boxplot arranged by hours, the daily pattern can be seen.
...
is cycle behaviour. Also it easy to see two outliers. It's better to
deal with them now.

```{r}
metro$temp[metro$temp < -200] <- NA

#replace NA with previous non-NA value
metro$temp <- na.locf(metro$temp)
```

Let's plot boxplot of the temperature.
...
repetitive monthly bahoivour can be seen.
...
Easy to notice that the most common weather description is "Clouds" and
the second by popularity is "Clear".

Stacked bar chart of weather proportions with holidays on x-axis.
...
ACF exhibits the strong seasonal behavior of traffic volume.
...
within temperature component.

```{r}
acf(metro$clouds_all, main = "ACF - Clouds")
```

ACF plot shows high autocorrelation within clouds_all column.
...
Can be noticed in some periodic behavior.
...
1.  From 2012-10-02 09:00:00 to 2013-10-02 09:00:00 the missing values
    would be imputed by agregation. For numerical values, the mean of
    the previous 2 values would be calculated and for categorical one
    the previous one would be assigned.
2.  For the rest of the data, the values would be impute from historical
    data available.
...
The amount of missing values for the rest of data:
...
Impute the rest of values by using historical data.
...
We're interested in checking that there are no correlation of the
forecast errorsto assess our model betterl. To capture this, we can use
the ACF function or LjungBox test or examine the histograms for residual
values. In R there is function checkresidual() that contain all these
actions in one.
...
shown below. However, it is not true for our model. For the Ljung-Box
...
The p values for the AR(p) model is then determined by when the PACF
drops to below significant threshold (blue area) for the first time.

ACF can be used for determining the q value. It is typically selected as
the first lagged value of which the ACF drops to nearly 0 for the first
time.

From the graphs, we can notice that p value should be 27 and q values
also should be 6. So we can use 3 models: ARMA(27, 0), ARMA(27, 6) and
ARMA(0, 6).
...
Based on done EDA, the traffic volume and temperature are seemed to be
cointegrated.

First thing to do is to test whether individual series are integrated.
...
The augmented Dickey-Fuller statistic is -14.3326, 102.7121, and do not
...
The augmented Dickey-Fuller statistic is -14.5137, 70.2169, 105.3253,
and do not lie inside the acceptance region at 1%, 5%, and 10%. p-value
\< 0.05. Therefore, we reject the presence of unit root
(non-stationarity).
...
The augmented Dickey-Fuller statistic is -12.5531, and do not lie inside
the acceptance region at 1%, 5%, and 10%. p-value \< 0.05. Therefore, we
reject the presence of unit root (non-stationarity).
...
Also Philips-Ouliaris test has confirmed the series are contegrated at
the level 1%. It justifies the use of a regression model.

Let's construct error correction model.
...
Error term is highly significant. **Conslusion**: traffic volume and
temperature are cointegrated variables.

## Multivariate time series analysis using VAR

Let's build vector autoregresive model using var function.
...
Unfornatly, the error is shown, that cannot allocate vector of size 20.6
Gb.

## Saving pre-processed data for futher work

Let's save pre-proccessed dataframe for further using it during
implementation of RNN and LSTM network at Python.

For no habing a problem with date_time column, it is better to save it
as a character. Because during saving in csv format that date like
